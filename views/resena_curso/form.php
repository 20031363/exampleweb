<div style="margin:5%;">
    <h1 class="generic-font" style="color: rgb(31, 43, 33)!important; font-weight:bolder;text-align:center;">
        <?php echo (isset($_GET['id'])) ? 'Modificar' : 'Nueva' ?> Evaluacion Materia:
    </h1>

    <form action="resena_curso.php?accion=<?php echo (isset($_GET['id'])) ? 'modificar&id=' . $_GET['id'] : 'crear' ?>" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group" style="margin:5%;">
            <label class="generic-font" style="color: rgb(31, 43, 33)!important; font-weight:bolder;text-align:center;font-size:3rem;" for="producto">Materia:</label>
            <select name="datos[id_curso]" id="producto" required>
                <?php foreach($productos as $producto): ?>
                    <option value="<?php echo $producto['id_curso']; ?>" <?php echo (isset($_GET['id']) && $info['id_curso'] == $producto['id_curso']) ? 'selected' : ''; ?>>
                        <?php echo ('Curso: '.$producto['curso']); ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div style="text-align:center;margin:5%;">
            ...
            <button type="button" onclick="window.location.href='./../admin/resena_curso.php'" class="btn btn-primary set_buton" style="background-color:red;padding:1rem 3rem; font-size:1.4rem;font-weight:normal;">Cancelar</button>
        </div>
    </form>
</div>
